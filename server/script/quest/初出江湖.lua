...
setQuestFSMTable(
{
        ...
        [[
            ...
            return
            {
                [SYS_ENTER] = function(uid, value)
                    dialog.post(uid, questPath,
                    {
                        '多亏了你，我才能及时画完所有的护身符啊！这是辛苦费请你收下！',
                        '再回去找找士官吧，或许还有别的事情要你做呢！',
                    },
                    dialog.link(SYS_EXIT, '结束'))
    ...
    quest_prepare_to_wang = function(uid, value)
        if uidRemoteCall(uid, [[ return getLevel() ]]) < 6 then
            ...
            [[
                ...
                {
                    [SYS_ENTER] = function(uid, value)
                        dialog.post(uid, questPath, '施主的实力真是大有所增啊！下次交给你做的事可能有点难，等你修炼到了6级再来吧！',
                        dialog.link(SYS_EXIT, '结束'))
                    end,
                }
            ]])
        ...
        [[
            ...
            return
            {
                [SYS_ENTER] = function(uid, value)
                    ...
                    dialog.link('npc_ask_who_are_you', '请问阁下是王大人吗？'))
                end,

                npc_ask_who_are_you = function(uid, value)
                    dialog.post(uid, questPath, '难道你是受道馆的士官之托而来的人？',
                    {
                        dialog.link('npc_grant_bonus', '正是在下！'),
                        dialog.link('npc_deny', '认错人了。'),
                    })
                end,
                ...
                npc_grant_bonus = function(uid, value)
                    dialog.post(uid, questPath,
                    {
                        ...
                        '这是给你的辛苦费，请收下吧！对了，或许以后还需要你的帮助呢，下次再来吧！',
                    },
                    dialog.link(SYS_EXIT, '结束'))
    ...
    quest_done_wang_book = function(uid, value)
        ...
        [[
            ...
            {
                [SYS_ENTER] = function(uid, value)
                    dialog.post(uid, questPath, '暂时没有什么需要你帮忙的。',
    ...
    setQuestHandler(questName,
    {
        [SYS_ENTER] = function(uid, value)
            ...
            if level < 4 then
                ...
                {
                    '你好？',
                    '如果你不了解传奇3，就让贫道简单地给你介绍一下吧。',
                },
                {
                    dialog.link('npc_what_is_taoist', '什么是道士？'),
                    dialog.link('npc_what_is_dogwan', '这道馆是什么地方？'),
                    dialog.link('npc_who_are_you', '士官是做什么事的人？'),
                    dialog.link(SYS_EXIT, '退出'),
                })
            else
                dialog.post(uid, questPath,
                {
                    '贫道会教你做一些简单但值得去做的事情，一边做一边慢慢的熟悉一下道馆内的事情！',
                ...
                },
                {
                    ...
                    dialog.link('npc_what_is_dogwan', '这道馆是什么地方？'),
                    dialog.link('npc_who_are_you', '士官是做什么事的人？'),
                    dialog.link('npc_what_is_my_quest', '什么是我要做的事情？'),
                    dialog.link(SYS_EXIT, '退出'),
                })
            end
        end,
