        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    ...
                    {
                        '不知盗走不死牌的犯人到底要做什么坏事儿，真担心啊！',
                        '去道馆见一下执客院住持<t color="red">书堂玄震</t>道士吧！',
                    },
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    ...
                    dialog.link('npc_tell_theft', '我是来找在比奇省衙门偷走不死牌的那个道士的！'))
                end,
                ...
                npc_promise = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '这是很久以前的事儿了！',
                        '有个聪明的年青人进入我们道馆门下。' ..
                        '由于才能和悟性十分出众，所以得到了门派中元老们的特别器重。' ..
                        '甚至到了打算让他做继任馆主的程度。' ..
                        '可是谁想到那个家伙加入我们道门却另有居心！' ..
    ...
    quest_report_thief = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和书堂玄震说话',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '为了抓住署箭那个家伙，我们会紧密配合的！',
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ]])
    ...
    quest_hunt_taoist = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和署箭对质',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '比我想象的来的还要早啊！',
                    dialog.link('npc_name_him', '你就是署箭道士！'))
                end,

                npc_name_him = function(uid, args)
                    dialog.post(uid, questPath, '竟然连这儿都知道~？真是像传闻中说的那样手腕高明啊！',
                    dialog.link('npc_demand', '我是来取回被你偷走了的不死牌的！'))
                end,

                npc_demand = function(uid, args)
                    dialog.post(uid, questPath, '哈哈哈！您可真是个奇怪的人啊!难道我会那么轻易的还给你？还是别痴心妄想了，赶快回去吧！',
                    dialog.link('npc_press', '别说大话，赶快拿出来！'))
                end,
    ...
    quest_taoist_fled = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和署箭对质',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '果然实力非凡啊！现在我终于相信你能独自打败半兽勇士的事儿了！',
                    dialog.link('npc_ask_purpose', '你偷走不死牌就只是为了制造这种怪物？'))
                end,
    ...
    quest_report_mine = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '禀报连接通路的遭遇',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '回来了啊！找到那个堕落道士的行踪了吗？',
                    dialog.link('npc_report_escape', '虽然在连接通路里碰到了他，可还是让他跑掉了！'))
                end,
                ...
                npc_report_escape = function(uid, args)
                    ...
                    dialog.link('npc_confirm_monster', '是的！好像都是一些腐烂的尸体似的东西，是从来没有见过的怪物！'))
                end,
                ...
                npc_go_mine = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '根据观察结果的通报，矿山中的怪物一共有5种形态：恶哑僵尸、独角僵尸和蚀豸僵尸都是死去的尸体复活后变成的一般僵尸，这些僵尸反复杀死几遍都会复活。',
                        '此外，叫法老侣僵尸的怪物因为生前是僧侣，所以还穿着僧侣的服装。叫做老道僵尸的怪物因为生前是法神，所以会使用魔法进行攻击。',
                        '不过幸运的是这后两种僵尸被打死后就不会再复活了。',
                    },
                    dialog.link('npc_thanks_info', '这真是很有用的情报啊！谢谢啦！'))
                end,

                npc_thanks_info = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '朝廷软弱无力，即使发生了如此的大事都要依靠百姓支持，真是惭愧惭愧啊！',
                        '只能给你这些帮助，还请原谅啊！',
                    },
                    dialog.link(SYS_EXIT, '结束'))
    ...
    quest_search_mine = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和署箭对质',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '又是你？',
                    dialog.link('npc_stop_him', '你不能马上停止这邪恶勾当吗？'))
                end,
                ...
                npc_stop_him = function(uid, args)
                    ...
                    dialog.link('npc_defy', '哼！我可不怕你制造的怪物！'))
                end,
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    ...
                    dialog.link('npc_wake_up', '别说大话了，还是清醒一下吧！'))
                end,

                npc_wake_up = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '哼！再试验一下的话就能掌握长生不老的力量了，没必要冒着危险跟你这样的人打架而自找麻烦！',
                        '今天就到此为止吧！我先走一步了！',
                    },
    ...
    quest_report_king = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '禀报矿山的遭遇',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '去矿山的事儿怎么样了？',
                    dialog.link('npc_report_escape', '虽然在矿山深处找到了署箭，可还是让他跑掉了！'))
                end,
                ...
                npc_ask_plan = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '以前你在连接通路里跟他碰面的时候，他不是说过要和您一起支配世界这样的大话嘛！',
                        '如果能把由拥有这样强大能力的怪物组成军队掌握在手中的话，就没有不能支配世界的理由了啊！',
                        '不管怎么样这可是个大事儿啊！',
                    },
                    dialog.link('npc_ask_next', '现在该怎么办才好呢？'))
                end,

                npc_ask_next = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '这次让这个家伙逃脱，估计短时间内他不会再轻易露面了。我们还是一边进行全面的警戒，一边准备合适的办法来对付他筹划的阴谋吧！',
    ...
    quest_tell_temple = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '关于道馆',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '把我的话转告给书堂玄震道士去吧！',
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ]])
    ...
    quest_temple_pledge = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和书堂玄震说话',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '请去比奇省转告一下吧！',
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ]])
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '得到了道馆方面将会尽全力来协力解决这件事儿的承诺，我也稍微能安下心来了！',
                        '看来寻找那个堕落道士行踪的时期要全力以赴，拼了命也要跟他周旋到底。',
                        '不管怎么样，您为解决因官府失误而丢失重要物品才引发的这件事情如此尽心尽力，我除了感谢的话之外都不知该说什么才好了！',
                    },
                    dialog.link('npc_take_reward', '这只是我理所应当做的而已啊！'))
                end,

                npc_take_reward = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '这是本官替朝廷赐给你的，虽然微薄但却表达了我诚意，请不必推辞务必收下啊！',
                        '请在要紧的时候用吧！',
                        '能够有如此协助官府工作的百姓，本官真的很满足啊！',
                    },
                    dialog.link('npc_hear_valley', '还有别的消息吗？'))
                end,
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    ...
                    dialog.link('npc_ask_matter', '有什么事儿吗？'))
                end,

                npc_ask_matter = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '昨天身体状态还可以，就带着鹤嘴锄进了矿山。差不多采完了近处质量较好的矿石后又进入了矿坑深处。像往常一样用鹤嘴锄采矿的时候，却不知从哪儿传来了奇怪的声音！',
    ...
    quest_report_lair = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '和蛇谷老矿夫说话',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '不死牌？署箭？你的话好像更是奇怪啊！',
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ]])
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '哦！你说已经找到了署箭那个家伙的藏身之所？终于知道了那个恶道的住处啦！',
                        '辛苦你了！不过那个家伙会用困魔咒，还是拿他无从下手啊！',
                        '好像还是需要他们的帮助才行啊！',
                    },
                    dialog.link('npc_ask_temple', '您是说让我去道馆吗？'))
                end,

                npc_ask_temple = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '是啊！去书堂玄震道士那儿问问是否有能够破解困魔咒的方法。',
    ...
    quest_ask_seal = function(uid, args)
        ...
        [[
            ...
            return
            {
                [SYS_LABEL] = '关于困魔咒',
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '对我们来说，已经没有余暇等到那个家伙做好充分的准备设下困魔咒出来的时候了！',
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ]])
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath, '快请进，有什么新的消息吗？',
                    dialog.link('npc_tell_seal', '找到了署箭的下落，可是那个地方由于用不死牌设下了困魔咒进不去啊'))
                end,
                ...
                npc_ask_way = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '贫道得知如果有矿山出现的怪物们中<t color="red">僧侣僵尸骨</t>和<t color="red">雷电僵尸骨</t>的话，或许可以造出破解署箭施下魔法的护身符来！',
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    ...
                    if not (hasMonkBone or hasThunkBone) then
                        dialog.post(uid, questPath, '需要僧侣僵尸骨和雷电僵尸骨！',
                        dialog.link(SYS_EXIT, '结束'))
                        return
                    end

                    if not (hasMonkBone and hasThunkBone) then
                        dialog.post(uid, questPath, '找到了僧侣僵尸骨，那就快去找雷电僵尸骨吧！',
                        dialog.link(SYS_EXIT, '结束'))
                        return
                    end
                    ...
                    dialog.link('npc_take_charm', '知道了！'))
                end,
        ...
        [[
            ...
            return
            {
                ...
                [SYS_ENTER] = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '我想道馆会有什么对付那家伙的办法的，毕竟这是关系到他们名声的事儿啊！',
                        string.format('一定要让署箭那个堕落道士死在不是部下，而是<t color="red">%s</t>您的手中啊！', server.player.getName(uid)),
                    },
                    dialog.link(SYS_EXIT, '结束'))
                end,
            }
        ...
        [[
            ...
            return
            {
                ...
                npc_ask_token = function(uid, args)
                    dialog.post(uid, questPath,
                    {
                        '尽管它是很有价值的东西，但结果却造成了这样的灾难，绝对不能把它留在世上了！',
                        '一定要想个办法尽早把它毁掉。这次不管云发先生怎么劝阻，都要按照我意思来办！',
                    },
                    dialog.link('npc_hand_token', '那我就问到这儿吧！'))
                end,
    ...
    setQuestHandler(questName,
    {
        ...
        [SYS_ENTER] = function(uid, args)
            if server.player.getQuestState(uid, prequestName) ~= SYS_DONE then
                dialog.post(uid, questPath, '这不是还没有完成被盗灵魂任务的你所应该关心的事儿！',
                dialog.link(SYS_EXIT, '结束'))
                return
            end
            ...
            if server.player.getLevel(uid) < minQuestLevel then
                dialog.post(uid, questPath,
                {
                    '不用担心不死牌也行啊！被我放在城内安全的保管着呢！',
                    string.format('你的等级还没有超过<t color="red">%d级</t>，这种事儿还轮不到你操心。', minQuestLevel),
                },
                dialog.link(SYS_EXIT, '结束'))
                return
            end
        ...
        npc_ask_clue = function(uid, args)
            dialog.post(uid, questPath, '既然是跟道士有关的事儿，道馆里应该很清楚吧？而且道士们中能做出如此没有道义之事儿的人应该不多，所以去道馆打听一下的话或许就能掌握那个犯人的下落呢！',
            dialog.link('npc_accept_quest', '好，那我先去道馆看看！'))
        end,
