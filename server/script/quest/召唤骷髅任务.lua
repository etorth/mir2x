    ...
    for _, talker in ipairs(talkers) do
        ...
        [[
            ...
            local function postLines(uid)
                local choices = {}
                if rude then
                    table.insert(choices, dialog.link('npc_rude', rude.label))
                end
                table.insert(choices, dialog.link(SYS_EXIT, reply, {close = true}))

                dialog.post(uid, questPath, lines,
                choices)
            end
            ...
            if rude then
                ...
                handlers.npc_rude = function(uid, value)
                    if (rude.chance > 1) and (math.random(rude.chance) ~= 1) then
                        dialog.post(uid, questPath, rude.spared,
                        dialog.link(SYS_EXIT, '结束'))
                        return
                    end

                    dialog.post(uid, questPath, rude.punish,
                    dialog.link(SYS_EXIT, rude.reply))
        ...
        return
        {
            ...
            [SYS_ENTER] = function(uid, value)
                if retry then
                    ...
                    {
                        dialog.link('npc_accept_duel', '好的，现在就开始。'),
                        dialog.link('npc_refuse_duel', '准备好了，再来！'),
                    })
                    return
                end
    ...
    [[
        ...
        return
        {
            ...
            [SYS_ENTER] = function(uid, value)
                dialog.post(uid, questPath,
                {
                    '如果没有回应召唤的<t color="red">守护灵</t>，即使明白了召唤骷髅的道理也没有任何作用。',
                    '再次去地牢空间吗？',
                },
                {
                    dialog.link('npc_retry', '好的，去地牢空间。'),
                    dialog.link('npc_explain', '地牢空间里要做什么？'),
            ...
            npc_not_yet = function(uid, value)
                dialog.post(uid, questPath, '这种情况下，做好<t color="red">万全的准备</t>还是很明智的。被称为地牢空间的地方是进进出出没有一块儿好地方的场所。',
                dialog.link(SYS_EXIT, '结束'))
            end,
            ...
            npc_explain = function(uid, value)
                dialog.post(uid, questPath,
                {
                    '如果想学习召唤骷髅，请到地牢空间和将要成为自己<t color="red">守护灵</t>的骷髅直接对决吧。',
                    '如果对决赢了，这个骷髅将成为你的守护灵。',
                },
                {
                    dialog.link('npc_enter_yard', '好的，去地牢空间。'),
                    dialog.link(SYS_EXIT, '结束'),
                })
            end,
...
[[
    ...
    local blurb = [=[召唤骷髅是将在地牢空间里彷徨<t color="red">战士的灵魂召唤回来，一起进行战斗的魔法</t>。]=] ..
    ...
    setQuestHandler(questName,
    {
        ...
        [SYS_ENTER] = function(uid, value)
            ...
            if server.player.hasMagic(uid, magicName) then
                ...
                {
                    '感受到保护你的正气了吧!',
                    '你已经修炼了<t color="red">召唤骷髅</t>，也没有必要在接受我的指教了。',
                },
                dialog.link(SYS_EXIT, '结束'))
                return
            end
            ...
            if server.player.getLevel(uid) < minQuestLevel then
                dialog.post(uid, questPath, string.format('%s你现在修炼的程度还不够，在修炼一下再来！', blurb),
                dialog.link(SYS_EXIT, '结束'))
                return
            end

            dialog.post(uid, questPath, blurb,
            dialog.link('npc_ask_teach', '请向我传授召唤骷髅的魔法吧！'))
        end,
        ...
        npc_ask_teach = function(uid, value)
            dialog.post(uid, questPath,
            {
                '好的，那么你利用守护灵帮助完成见到<t color="red">战士灵魂</t>的合同。但是我能做的就是使你可以遇见战士们的灵魂送你去地牢空间，那以后的事情你要自己看着处理。',
                '如果搞不好，就永远无法从地牢空间回来，并且有可能和他们一样成为彷徨的灵魂。',
                '喔，怎么办？',
            },
            {
                dialog.link('npc_accept', '没关系，请送到地牢空间。'),
                dialog.link('npc_not_yet', '准备好了，再来！'),
            })
        ...
        npc_not_yet = function(uid, value)
            dialog.post(uid, questPath, '说实话是很危险的地方，但是只有冒险也才可以得到东西。请做好<t color="red">完备的准备</t>，再来！',
            dialog.link(SYS_EXIT, '结束'))
        end,
