...
--     end
-- }
local RSVD_NAME_defaultChatEventHandlers = nil
...
-- | 询问商人的灵魂                                                       |
-- | 询问王寡妇的剪刀                                                     |
-- | 随便聊聊                                                             | <-- display this line if RSVD_NAME_defaultChatEventHandlers valid
-- |                                                                      |
-- | 退出                                                                 |
-- +---------------------------------------------------------------------(x)
--
...
local RSVD_NAME_passiveQuestEventHandlers = nil
...
--
-- 2. all these tables of handlers are dynamically generated, and saved in this RSVD_NAME_uidActivedEventHandlers, not in database
--    so when server reboot, all these tables of handlers need to be re-generated, this is done by quest system enter function
local RSVD_NAME_uidActivedEventHandlers = nil
...
function setEventHandler(eventHandler)
    if RSVD_NAME_defaultChatEventHandlers ~= nil then
    ...
    RSVD_NAME_defaultChatEventHandlers = eventHandler
end

function hasEventHandler(event)
    if event == nil then
        return RSVD_NAME_defaultChatEventHandlers ~= nil
    end
    ...
    assertType(RSVD_NAME_defaultChatEventHandlers, 'table')
    if RSVD_NAME_defaultChatEventHandlers[event] == nil then
        return false
    end

    assertType(RSVD_NAME_defaultChatEventHandlers[event], 'function')
    return true
end
...
function RSVD_NAME_coth_main(uid)
    ...
    if event ~= SYS_NPCDONE then
        if hasEventHandler(event) then
            RSVD_NAME_defaultChatEventHandlers[event](from, value)
        else
            uidPostXML(uid,
            [[
                <layout>
