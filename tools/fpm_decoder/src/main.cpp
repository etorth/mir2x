 ...
 *  Last Modified: 07/12/2017 18:49:20
 *
 *    Description: decode re_xue_sha_chen fpm file to all files it includes
 *
 *                      fpm_decoder magic_str full_fpm_path out_dir
 *
 *                 i.e.
 *                  
 *                      fpm_decoder codudu.fpm amp/login.apm home/you/out
 *
...
{
    std::printf("%s",
            "Usage: decode re_xue_sha_chen fpm file to all files it includes\n"
            "\n"
            "           fpm_decoder magic_str full_fpm_path out_dir\n"
            "\n"
            "       magic_str should be cocodu.fpm, i.e.\n"
            "\n"
            "           fpm_decoder codudu.fpm amp/login.apm home/you/out\n"
            "\n"
            "       will print all needed information during extracting\n");
...
int fmp_Decode(const char *szMagicStr, const char *szFPMFileName, const char *szOutDir)
{
...
int main(int argc, char *argv[])
{
    ...
    if(std::strcmp(argv[1], "codudu.fpm")){
        printUsage();
        return 1;
    }

    return fmp_Decode(argv[1], argv[2], argv[3]);
}
