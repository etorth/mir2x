 ...
 *  Last Modified: 05/21/2017 01:33:26
 *
 *    Description: 
 *
 *        Version: 1.0
    ...
    , m_AnimationState(0)
{
    ...
    int nAnimationSet   = (nAnimationIndex / 3000);
    int nAnimationState = (nAnimationIndex % 3000) / 80;
    if(false
            || nFileIndex      == 0    // 1, 2, ..., 20 for Mon-1, ..., Mon-20
            || nFileIndex      >  2
            // || nAnimationIndex >= g_ValidAnimationWindow->AnimationCount()
            || nAnimationSet   >= g_ValidAnimationWindow->AnimationCount()
            || nAnimationState >  32
      ){
        m_FileIndex      = 1;
        m_AnimationState = 0;
        ...
        m_AnimationState = nAnimationState;
        m_AnimationIndex = nAnimationSet; // we have 0 ~ 8 animations
                                          // each has 0 ~ 32 states
    }
    ...
    for(int nFrame = 0; nFrame < 10; ++nFrame){
        int nBaseIndex = m_AnimationIndex * 3000 + m_AnimationState * 80 + 51 + nFrame;
        ...
        if(true
            ...
            // TODO add it back when hand-made shadow algorithm is available
            ...
            {// non-shadow layer
                ...
                //5: direction
                //6: frame index
                std::sprintf(szSaveFileName,
                        "./IMG/0%02d%02d%02d5%02d.PNG",
                        m_FileIndex, m_AnimationIndex, m_AnimationState, nFrame);
            ...
            //         }
            //         g_WilImagePackage[1].Decode(pData, 0XFFFFFFFF, 0XFFFFFFFF);
            //         SaveRGBABufferToPNG((uint8_t *)pData,
            ...
            nMaxW = (std::max)(nMaxW, m_PNG[0][nFrame]->w());
            nMaxH = (std::max)(nMaxH, m_PNG[0][nFrame]->h());
...
void AnimationPreviewWindow::RedrawAll()
{
    ...
    // int nMinX = 0;
    // int nMinY = 0;
    // int nMaxX = m_PNG[0][m_CurrentFrameIndex]->w();
    // int nMaxY = m_PNG[0][m_CurrentFrameIndex]->h();
    ...
    // int nMaxW = nMaxX - nMinX;
    // int nMaxH = nMaxY - nMinY;
