...
bool AnimationSet::ImportMir2Animation(int nFileIndex, int nAnimationIndex)
{
    ...
    for(int nDirection = 0; nDirection < 8; ++nDirection){
        ...
        for(int nStatus = 1; nStatus < 33; ++nStatus){
...
bool AnimationSet::Export()
{
    ...
    if(g_SIDWindow->BaseLID() < 0 || g_SIDWindow->BaseLID() >= 9){
        g_SIDWindow->ShowAll();
        return false;
    }
            ...
            if(m_ActionSet[nStatus][nDir].Valid() && m_ActionSet[nStatus][nDir].FrameCount() > 0){
                { // for offset
                    ...
                    m_ActionSet[nStatus][nDir].Export(
                            ...
                            g_SIDWindow->BaseLID() * 2 + g_SIDWindow->Gender(),
                            nStatus,
                            nDir,
                            m_DX,
                            m_DY,
                ...
                { // for directive rectangle cover
                    ...
                    // fprintf(pFile, 
                    //         "%d,%d,%d,%d,%d\n",
                    //         g_SIDWindow->BaseLID() * 2 + g_SIDWindow->Gender(), nStatus, nDir, nW, nH);
...
void AnimationSet::DSetDynamicShadowOffset(bool bReset, int nDX, int nDY)
{
    m_ActionSet[m_Status][m_Direction].DSetDynamicShadowOffset(bReset, nDX, nDY);
}
