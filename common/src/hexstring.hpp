 ...
 *  Last Modified: 03/17/2016 22:14:23
 *
 *    Description: 
 *
 *        Version: 1.0
...
#include <type_traits>
        ...
        // sizeof(KeyT) is defined by declaration
        // nUseByteNum = 0: use all bytes
        // otherwise, use std::min(sizeof(KeyT), nUseByteNum) from LSB
        //
        // 1. invocation should prepared enough buffer to szString
        // 2. no '\0' at the end, be careful

        template<size_t ByteN = 0> static const char *HexString(KeyT nKey, char *szString)
        {
            const size_t nByteN = (ByteN) ? sizeof(KeyT) : std::min(ByteN, sizeof(KeyT));
