...
struct SMAuctionBuyError
{
    uint8_t error;
};

struct SMAuctionUnregisterError
{
    uint8_t error;
};
...
#define _RSVD_register_servermsg(type, ...) result.at(type) = std::make_unique<msgf::MsgAttribute>(#type, __VA_ARGS__)
        ...
        _RSVD_register_servermsg(SM_AUCTIONBUYERROR,        1, sizeof(SMAuctionBuyError)       );
        _RSVD_register_servermsg(SM_AUCTIONUNREGISTERERROR, 1, sizeof(SMAuctionUnregisterError));
