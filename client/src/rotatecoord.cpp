 ...
 *        Created: 8/15/2015 4:01:57 PM
 *  Last Modified: 08/17/2015 5:47:34 AM
 *
 *    Description: 
 *
 *        Version: 1.0
...
static bool PointInSegment(int nX, int nStartX, int nW)
{
    return nX >= nStartX && nX < nStartX + nW;
}

static bool PointInRect(int nX, int nY, int nStartX, int nStartY, int nW, int nH)
{
...
bool RotateCoord::Set(int nCenterX, int nCenterY, int nStartX, int nStartY, int nW, int nH)
{
    if(nW > 0 && nH > 0){
        m_CenterX = nCenterX;
        m_CenterY = nCenterY;
        ...
        if(PointInRect(m_CenterX, m_CenterY, m_StartX, m_StartY, nW, nH)){
            m_Distance  = 0;
            m_Direction = 0;
    ...
    if(m_Distance != 0){
        switch(m_Direction){
            case 0:
                {
                    if(m_CurrentX == (std::min)(m_StopX, m_CenterX + m_Distance)){
                        if(m_Overlap[1]){
                            ...
                            if(PointInRect(nNewX, nNewY, m_StartX, m_StartY,
            ...
            case 3:
                {
                    if(m_StopY >= m_CenterY + m_Distance){
						// may jump to the end of D3 when invoking Set()
                        // I have updated Set(), for d 3 to get the smallest direction
