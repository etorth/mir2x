...
void LocalSpeakBoard::addLog(const char8_t * format, ...)
{
    std::u8string text;
...
void LocalSpeakBoard::updateDefault(double)
{
    if(m_showTime > 0){
...
void LocalSpeakBoard::drawDefault(Widget::ROIMap m) const
{
    if(!m.calibrate(this)){
        return;
    }
