...
{
    if(!g_clientArgParser->disableAudio){
        // Unhook stopped-callbacks first.
        // MIX_SetTrackStoppedCallback takes the track's SDL_LockAudioStream, which is the same lock the audio thread holds while inside recycleSoundEffectTrack;
        // so this call blocks briefly if a callback is in flight, and afterwards no future callback can fire for these tracks.
        // Only THEN is it safe to clear m_freeTrackList / m_trackStateList without racing an async insert.
        for(auto *t : m_tracks){
...
void SDLDevice::flipWindowMaximized(std::optional<bool> maximized)
{
    fflassert(m_window);
    if(getWindowMaximized()){
        if(maximized.has_value() && maximized.value()){
            return;
        }

        if(!SDL_RestoreWindow(m_window.get())){
            throw fflpanic("SDL_RestoreWindow({:p}) failed: {}", to_cvptr(m_window), SDL_GetError());
        }
    }
    else{
        if(maximized.has_value() && !maximized.value()){
        ...
        if(!SDL_MaximizeWindow(m_window.get())){
            throw fflpanic("SDL_MaximizeWindow({:p}) failed: {}", to_cvptr(m_window), SDL_GetError());
        }
    }
}
...
void SDLDevice::scaleMaximized(std::optional<float> scale)
{
    if(!getWindowMaximized()){
        return;
    }
...
{
    if(getWindowMaximized() || getWindowFullscreen()){
        return;
    }
